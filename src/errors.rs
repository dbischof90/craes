...
type OrderReaderError = sync::mpsc::SendError<(
    order::Order,
    sync::oneshot::Sender<std::result::Result<std::vec::Vec<order::UnconditionalOrder>, AssetHandlingError>>,
)>;
#[derive(Debug)]
...
pub enum DatabaseChannelError {
    Trades(sync::mpsc::SendError<HashMap<order::Order, Vec<order::UnconditionalOrder>>>),
